...
import 'dart:convert';
import 'package:dio/dio.dart'; // Usando Dio para las peticiones HTTP
...
class _PokemonListState extends State<PokemonList> {
  ...
  String _ipInfo = '';
  final dio = Dio(); // Inicializamos Dio
  ...
  Future<void> fetchPokemon() async {
    ...
    final url = 'https://pokeapi.co/api/v2/pokemon?limit=50';
    try {
      final response = await dio.get(url); // Usamos dio para hacer la solicitud
      if (response.statusCode == 200) {
        ...
        for (var pokemon in results) {
          final detailsResponse = await dio
              .get(pokemon['url']); // Usamos dio para la solicitud detallada
  ...
  Future<void> _fetchIpInfo() async {
    ...
    final apiKey =
        '5bfa51c315854c6f8e968cc048c7fb6b'; // Reemplaza con tu API Key de ipstack
    ...
    try {
      final response = await dio.get(url); // Usamos dio para la solicitud
      if (response.statusCode == 200) {
        final data = response.data;
        setState(() {
          _ipInfo = '''
